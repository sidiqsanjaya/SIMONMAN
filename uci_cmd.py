...
def set_static_dhcp(name, ip, mac):
    configurations = [
        "uci add dhcp host",
        "uci set dhcp.@host[-1].name="f"{name}",
        "uci set dhcp.@host[-1].ip="f"{ip}",
        "uci set dhcp.@host[-1].mac="f"{mac}",
        "uci commit dhcp"
    ]
...
def get_static_dhcp():
    ...
    while True:
        try:
            dhcp = u.get_all(f"dhcp.@host[{no}]")
            host[dhcp.get('ip')] = {}
            host[dhcp.get('ip')]['mac'] = dhcp.get('mac')
            host[dhcp.get('ip')]['name'] = dhcp.get('name')
            no = no + 1
        except:
            break
    return host
